            ...
            <td>
              <?php if ($d['statut'] === 'en_attente'): ?>
                <div class="action-btns">
                  <?= form_open('admin/demandes/approuver/' . $d['id'], ['style' => 'display:inline']) ?>
                    <?= csrf_field() ?>
                    <button type="submit" class="btn-sm btn-approve">
                      <i class="bi bi-check-lg"></i> Approuver
                    </button>
                  <?= form_close() ?>
                  <button class="btn-sm btn-refuse" onclick="ouvrirRefus(<?= $d['id'] ?>, '<?= esc($d['prenom']) ?> <?= esc($d['nom']) ?>')">
                    <i class="bi bi-x-lg"></i> Refuser
                  </button>
                </div>
              <?php else: ?>
                <span class="td-muted" style="font-size:.75rem">
                  <?= !empty($d['traite_par']) ? 'par ' . esc($d['traite_par']) : '—' ?>
                </span>
              <?php endif; ?>
            </td>
          </tr>
...
<div id="modal-refus" style="display:none;margin-top:1.5rem">
  <div class="form-section" style="border-color:var(--danger-br);background:var(--danger-bg)">
    ...
    <?= form_open('admin/demandes/refuser', ['id' => 'form-refus']) ?>
      <?= csrf_field() ?>
      <input type="hidden" name="demande_id" id="refus-id"/>
      <div class="f-group">
        <label class="f-label">Commentaire pour l'employé (optionnel)</label>
        <textarea name="commentaire" class="f-textarea" placeholder="Motif du refus..."></textarea>
      </div>
...
function filtrer(btn, statut) {
  ...
  btn.style.background='var(--forest)'; btn.style.color='var(--white)'; btn.style.borderColor='var(--forest)';
